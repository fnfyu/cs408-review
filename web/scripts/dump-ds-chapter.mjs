...
const lessons = org.lessons.filter((l) => l.subject === 'ds' && l.chapter_order === chapterOrder);
...
for (const l of lessons) {
  ...
  if (!a?.page_texts) {
    out += `(missing page_texts in all-lessons)\n\n`;
    continue;
  }
...
const outPath = path.join(root, `_ds_ch${chapterOrder}_dump.txt`);
