...
const contentDir = path.join(root, 'content/ds');

const EXAMPLE_KINDS = new Set(['例题与应用', '公式/代码/计算']);

function loadPatches() {
  // ch1.json, ch6a.json, ch6b_mst.json, ch7a.json ...
  const files = fs
    .readdirSync(contentDir)
    .filter((f) => /^ch\d+[a-z]?(?:_[a-z0-9]+)?\.json$/i.test(f))
    .sort();
  const map = new Map();
...
db.stats.ds_rewritten_lessons = [...patches.keys()].filter((id) =>
  ...
  JSON.stringify(
    {
      ...
      ds_rewritten_lessons: db.stats.ds_rewritten_lessons,
    },
    null,
    2,
  ),
